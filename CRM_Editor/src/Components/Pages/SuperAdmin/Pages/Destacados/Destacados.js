...
import {
  getAdminRecibirVideosComprados,
  getAdminRecibirVideosDestacados,
  getAdminRecibirVideosGuardados,
} from "../../../../../Actions/SuperAdmin/adminRecibirVideos";
...
export default function Destacados(props) {
  ...
  const videosGuardados = props.videosDestacadosObj;
  const imagenPerfilVideo = props.imagenDestacadosObjPerfil;
  ...
  useEffect(() => {
    if (datosUser != null) {
      ...
      if (filtrosActivados === true || videoEliminado == true) {
        dispatch(getAdminRecibirVideosDestacados(dataVideo));
