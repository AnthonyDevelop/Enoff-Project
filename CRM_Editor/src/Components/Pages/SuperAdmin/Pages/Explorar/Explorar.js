...
import { getAdminRecibirVideos } from "../../../../../Actions/SuperAdmin/adminRecibirVideos";

import { getUserUtilizoFiltros } from "../../../../../Actions/SuperAdmin/userUtilizoFiltros";
import "./Explorar-SuperAdmin.css";
//Firebase
// import {
//   getStorage,
//   ref,
...
export default function Explorar(props) {
  ...
  const videosGuardados = props.videosExplorarObj;
  const imagenPerfilVideo = props.imagenExplorarObjPerfil;
  ...
  const [videosActualizados, setVideosActualizados] = useState(false);
  ...
  useEffect(() => {
    if (datosUser != null) {
      ...
      if (filtrosActivados === true || videosActualizados == true) {
        dispatch(getAdminRecibirVideos(dataVideo));
        setVideosActualizados(false);
  ...
  }, [filtrosActivados, videosActualizados]);

  //Lista de Categorias
  // var valorDefectoCategoria = {
  //   value: "",
  //   label: "Seleecione una categoria",
  ...
  if (misVideos.data != null) {
    var localidades = [];
    // localidades.push(valorDefectoLocalidades);
  ...
  //   setTarjeta({ focus: e.target.name });
  // }

  const handleInputChange = (e) => {
    const { name, value } = e.target;

    setTarjeta({ [name]: value });
  };
            ...
            <>
              <div className="contenedor-filtros-titulo-buscador">
                ...
                <div className="box-input-buscador">
                  {/*ACA VA UN SELECT PICKER EN LA 2.0 PIDIENDO LA LISTA DE TITULOS DE TODOS LOS VIDEOS*/}
                  <InputGroup>
                    <Input
                      value={buscador}
                      ...
                      >
                        ...
                        <div className="vidio-header-info">
                          <Avatar
                            ...
                            alt="user"
                          />
                          <p className="font-nombre-usuario-vedor-reproductor-super-admin">
                            {img.nombreUserCreador}
                          </p>
                        </div>
                        <div className="vista-admin-sin-hover-video  ">
                          <p className="text-localidad-video">
                            <Rate
                              size="xs"
                              className="calificacion-video-stars-superadmin"
                              readOnly
